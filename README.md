...
| `POST /fhir/smart/import-patient` | Import the patient in the active SMART session: reads `Patient`, `Observation` and `DiagnosticReport` from the session's `iss` with the session token, then runs them through the **same** ingest + re-score path as a pasted Bundle. 401 without a usable session; 422 when the chart is readable but unmappable. An imported chart is filed under a NeuroPilot subject id (`FHIR-0001`, …) and keeps the EHR's own id as provenance (`external_ids.ehr_patient_id` + `ehr_iss`), which the record view shows — renamed for readability, never disguised |
